...
//		resp.then().log().status();
	...
	public void user_validate_the_id_is_not_null(){
		 ...
		 * Date date = new Date(); String todatesDate= dateFormat.format(date);
		 * 
		 * String createdAtValue= resp.getBody().jsonPath().getString("createdAt");
		 * Assert.assertTrue(createdAtValue.contains(todatesDate));
		 */
	}
	
	@Then("^user validate the current date$")
	public void user_validate_the_current_date() throws Throwable 
	{
		  DateFormat dateFormat= new SimpleDateFormat("yyyy-MM-dd");
		  Date date = new Date(); String todatesDate= dateFormat.format(date);
